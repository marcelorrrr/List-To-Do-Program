...
def obtener_todas():
    ...
    return [Tarea(id=t['id'], titulo=t['titulo'], descripcion=t['descripcion'], completada=bool(t['completada'])) for t in tareas_db]

def obtener_por_id(tarea_id):
    tarea_db = database.obtener_tarea_por_id(tarea_id)
    if tarea_db:
        return Tarea(id=tarea_db['id'], titulo=tarea_db['titulo'], descripcion=tarea_db['descripcion'], completada=bool(tarea_db['completada']))
    return None
